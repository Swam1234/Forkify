...
class RecipeView extends View {
  ...
  _errorMessage = 'No recipe is found! Try another one.'
  _message;
  ...
  addUpdateServingHandler(handler) {
    this._parentEl.addEventListener('click', function (event) {
      const updateTo = +event.target.closest('.btn--increase-servings')?.dataset.updateTo;
      if (!updateTo) return;
      handler(updateTo);
    })
  }
  ...
  _generateMarkup() {
   ...
   <div class="recipe__details">
     ...
     <button class="btn--round">
       <svg class="">
         <use href="${icons}#icon-bookmark${this._data.isBookmarked ? '-fill' : ''}">icon</use>
       </svg>
     </button>
   </div>
